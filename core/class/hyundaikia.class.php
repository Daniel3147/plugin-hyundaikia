...
class hyundaikia extends eqLogic {
    ...
    public static function dependancy_info() {
        ...
        // Vérifie jeedomdaemon
        $check = shell_exec('pip3 show jeedomdaemon 2>&1');
        if (strpos($check, 'Name:') === false) {
            $return['state'] = 'nok';
            return $return;
        }

        // Vérifie hyundai_kia_connect_api et sa version
        ...
        if (preg_match('/^Version:\s*(\S+)/m', $check, $m)) {
            $installed = $m[1];
            if (version_compare($installed, self::HKCA_MIN_VERSION, '<')) {
                log::add('hyundaikia', 'warning',
                    'hyundai_kia_connect_api v' . $installed .
    ...
    public static function dependancy_install() {
        ...
        $script       = dirname(__FILE__) . '/../../resources/post_install.sh';

        // Lance le script post_install.sh qui force la version >= 4.14.1
        $cmd  = 'sudo bash ' . escapeshellarg($script);
        $cmd .= ' >> ' . escapeshellarg($logFile) . ' 2>&1 &';
        $cmd .= ' echo $! > ' . escapeshellarg($progressFile);
        exec($cmd);
